...
function relativeRequire(m) {
	if (m.charAt(0) == '.' || m.charAt(0) == '/') {
		m = path.join(path.dirname(this), m);
	}
	return require(m);
}
			...
			if(service.method) {
				if(route.path && route.callback) {
                    ...
                    if(!multiplePathsCallbacks[service.name]) {
                        ...
                        logger.warn('Callback ' + route.callback + ' has already ' + times + ' path' + (times>1?'s, ':', ')
                            + 'name function as ' + service.name + times);
...
function checkFile(projectdir, fileName, main) {
    try {
        if(main) {
            var file = path.resolve(projectdir, fileName);
        } else {
            var file = path.resolve(projectdir, 'controllers', fileName + '.js');
        }
