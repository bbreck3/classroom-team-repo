...
exports.run = function(args, opts) {
    ...
    var parseResult = function(err, unzippedBody) {
        ...
        if( result.errcode && result.errcode == 100 ) {
            ...
            console.log();
            var len = result.stats.length;
            for( var i = 0; i < len; i++ ){
                var stat = result.stats[i];
                var logTime = new Date(stat.date);
                var fmt = u.lpad(logTime.getUTCMonth() + 1, 2, '0') + '/'
                    + u.lpad(logTime.getUTCDate(), 2, '0') + '/' + logTime.getUTCFullYear() + ' '
                    + u.lpad(logTime.getUTCHours(), 2, '0') + ':'
                    + u.lpad(logTime.getUTCMinutes(), 2, '0') + ':'
                    + u.lpad(logTime.getUTCSeconds(), 2, '0') + '.'
                    + u.lpad(logTime.getUTCMilliseconds(), 3, '0');

                console.log(('[' + fmt + ']').red + '\t'
                    + (opts.show_serverid?(('[Server:' + (stat.serverid===undefined?'Unassigned]\t\t':stat.serverid+']')).yellow + '\t'):'')
                    + 'memory: { free: ' + (stat.process.memory.free/1048576).toFixed(2) + 'M total: ' + stat.process.memory.total/1048576 + 'M }\t'
                    + 'loadavg: [ ' + (stat.os.load[0]*100).toFixed(2) + '% ]');
            }

            if(logTime) {
                u.saveLastLogTime(LOG_TYPE, startLogDateTimeStamp, endLogDateTimeStamp, stat.date);
                console.log();
            } else {
