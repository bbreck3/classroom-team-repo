var fs = require('fs'),
	...
	b64_hmac_sha1 = require('../oauth/sha1.js').b64_hmac_sha1,
	OAuth = require('../oauth/oauth.js').OAuth,
	...
	io,
	VARIANCE = 60, 	// set this to a variance we're willing to deal with in seconds -- this is 1m
	server,
	app = null,
	main_exts = ['.js'],
...
function loadConfig(projectDir, app, io, baseSandbox) {
	...
	if(u.exists(configure)) {
		...
		if(u.exists(controllersPath)) {
			...
			for(var i = 0; i < files.length; i++) {
				try {
					...
					var filePath = path.join(controllersPath, files[i]);
					var sandbox = getSandbox(baseSandbox, {dir: controllersPath, file: filePath, projectDir: projectDir});
                    sandbox.global = sandbox.GLOBAL = globalProxy;
                    var context = vm.createContext(sandbox);
					var code = fs.readFileSync(filePath, "utf8");
					var err = syntaxCheck(code, '/controllers/' + files[i]);
					if (err) {
					    u.die('Syntax error detected. ' + err);
					}
		...
		if(u.exists(filtersPath)) {
			var files = fs.readdirSync(filtersPath);
			for(var i = 0; i < files.length; i++) {
				try {
					var name = files[i].substring(0, files[i].indexOf('.js'));
					var filePath = path.join(filtersPath, files[i]);
					var sandbox = getSandbox(baseSandbox, {dir: filtersPath, file: filePath, projectDir: projectDir});
                    sandbox.global = sandbox.GLOBAL = globalProxy;
                    var context = vm.createContext(sandbox);
					var code = fs.readFileSync(filePath, "utf8");
					var err = syntaxCheck(code, '/filters/' + files[i]);
					if (err) {
					    u.die('Syntax error detected. ' + err);
					}
		...
		if(u.exists(websocketsPath)) {
			var files = fs.readdirSync(websocketsPath);
			for(var i = 0; i < files.length; i++) {
				try {
					var name = files[i].substring(0, files[i].indexOf('.js'));
					var filePath = path.join(websocketsPath, files[i]);
					var sandbox = getSandbox(baseSandbox, {dir: websocketsPath, file: filePath, projectDir: projectDir});
                    sandbox.global = sandbox.GLOBAL = globalProxy;
                    var context = vm.createContext(sandbox);
					var code = fs.readFileSync(filePath, "utf8");
					var err = syntaxCheck(code, '/websockets/' + files[i]);
					if (err) {
					    u.die('Syntax error detected. ' + err);
					}
		...
		if(filtersConfig && _.isArray(filtersConfig)) {
			for(var index in filtersConfig) {
				var map = filtersConfig[index];
				if(map && map.path && map.callback) {
					var name = map.callback.split('#')[0];
					var func = map.callback.split('#')[1];
		...
		if(routesConfig && _.isArray(routesConfig)) {
			for(var index in routesConfig) {
				var map = routesConfig[index];
				if(map && map.path && map.callback) {
					var method = '';
					if(!map.method) {
						method = 'get';
		...
		if(websocketsConfig && _.isArray(websocketsConfig) && websocketsConfig.length > 0) {
			io.on('connection', function(socket) {
				...
				for(var index in websocketsConfig) {
					var map = websocketsConfig[index];
...
function validateAuth(req, res, next) {
	...
	var appConsumerKey = service.consumerKey;
	var appConsumerSecret = service.consumerSecret;

	// no security is required if we don't have a key and secret
	if (!appConsumerKey && !appConsumerSecret) {
		return next();
	}
	...
	var parts = auth.substring(6).split(',');
	var map = {};
	...
	if (map['oauth_signature_method'] != 'HMAC-SHA1') {
		// we require HMAC
		return res.send(400);
	}
	if (map['oauth_version'] != '1.0') {
		// we only support 1.0
		return res.send(400);
	}
	var oauth_consumer_key = map.oauth_consumer_key;
	if (oauth_consumer_key !== appConsumerKey) {
		// make sure the incoming key and our mapped key match!
		return res.send(401);
	}
	var oauth_signature = map.oauth_signature;
	var oauth_nonce = map.oauth_nonce;
	var oauth_timestamp = map.oauth_timestamp;
	var method = req.method.toUpperCase();
	var url = req.protocol + '://' + req.headers['host'] + req.url;
	var options = urllib.parse(url);
	var scheme = options.protocol.substring(0,options.protocol.length-1).toLowerCase();
	var authority = options.host;
	var dropPort = (scheme == "http" && options.port == 80) || (scheme == "https" && options.port == 443);
	if (dropPort) {
	...
	var path = options.pathname || '/'; // conforms to RFC 2616 section
	// 3.2.2
	// we know that there is no query and no fragment here.
	var requestURL = scheme + "://" + authority + path;
	var list = req.headers['content-type'].indexOf('json') > 0 ? [] : OAuth.getParameterList(req.body);
	for (var k in map) {
		var v = map[k];
		if (!v) continue;
		list.push([k,v]);
	}
	list.push(['oauth_consumer_key', oauth_consumer_key]);
	var sortable = [];
	for (var p = 0; p < list.length; ++p) {
		var nvp = list[p];
		if (nvp[0] != "oauth_signature") {
			sortable.push([ OAuth.percentEncode(nvp[0])
			                + " " // because it comes before any character
									// that can
			                // appear in a percentEncoded string.
	...
	var encodedParams = OAuth.formEncode(sorted);
	var sig = OAuth.percentEncode(method) + '&' + OAuth.percentEncode(requestURL) + '&' + OAuth.percentEncode(encodedParams);
	var privateKey = OAuth.percentEncode(appConsumerSecret) + '&' + OAuth.percentEncode('');
	var checkSignature = OAuth.percentEncode(b64_hmac_sha1(privateKey, sig, '='));

	if (oauth_signature !== checkSignature) {
		return res.send(401);
	}

	var now = OAuth.timestamp();
	var delta = now - oauth_timestamp;
	if (delta > VARIANCE) {
		return res.send(401);
	}

	// check the nonce and make sure it's not being used for a replay attack
	/*
	 * if (!storageAdapter.checkNonce(oauth_timestamp, oauth_nonce, VARIANCE)) {
	 * return res.send(401); }
	 */
