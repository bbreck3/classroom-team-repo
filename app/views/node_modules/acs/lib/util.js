...
function checkEnvVarName(varName) {
    ...
    var blackList = ['appid', 'basedir', 'name', 'version', 'dirname', 'fullpath', 'framework', 'config', 'serverId', 'bodyParser', 'customConfig', 'HOME', 'NODE_PATH', 'USER', 'TMPDIR', 'PATH', 'NODE', 'PWD', 'port', 'PORT'];
...
function password (prompt, mask, callback) {
    ...
    function raw(mode) {
        var setRawMode = process.stdin.setRawMode || tty.setRawMode;
...
function getHost(args, hostname, prompt, cb) {
    ...
    if(typeof prompt === 'function') {
        cb = prompt;
        prompt = 'host (' + (this.getGlobalConfig().publishHost || this.getDefaultConfig().publishHost).cyan.bold + '):';
    }
    if(!hostname) {
    	program.prompt(prompt.white+' ', cb);
    } else {
...
function getConnectionErrorMsg(error, resBody) {
    var host_port = (getGlobalConfig().publishHost || getDefaultConfig().publishHost) + ':'
        + (getGlobalConfig().publishPort || getDefaultConfig().publishPort);
