...
program.command('new <name>')
    ...
    .option('--org <orgid>', 'create app for the specified organization. This is for enterprise user only.')
    .option('--env <envid>', 'create app for the specified environment. This is for enterprise user only.')
    .action(function() {
...
program.command('add <name>')
    .description('\tadd a new service to an existing app')
    .action(function() {
        restoreParams(this);
    });
...
program.command('list [appname]')
    ...
    .option('--mine', 'show apps created by the developer only. This is for enterprise user only.')
    .option('--org <orgid>', 'show apps of specified organization only. This is for enterprise user only.')
...
program.command('server [appname]')
    .description('\tcheck how many servers an org has and how many free ones are available or set the server size for an app')
...
function ccommander() {
    ...
    function hide(cmdName, option) {
        switch(cmdName) {
        case 'list':
            if(option.flags.indexOf('--id') !== -1) return true;
            break;
        case 'config':
            if(option.flags.indexOf('--inc') !== -1 || option.flags.indexOf('--dec') !== -1) return true;
            break;
        case 'cname':
            if(option.flags.indexOf('--path') !== -1 || option.flags.indexOf('--check') !== -1) return true;
            break;
        case 'publish':
            if(option.flags.indexOf('--git') !== -1) return true;
            break;
        case 'server':
            if(option.flags.indexOf('--list') !== -1) return true;
            break;
        }
        return false;
    }
